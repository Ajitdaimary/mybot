...
async def on_reaction_add(reaction, user):
  if reaction.message.server is None:
      ...
      if reaction.emoji == '⚙':
        ...
        embed.add_field(name = 'SEE',value ='``!!dm @user <text>``, ``!!muteinchannel @user <time in minutes>``, ``!!setupwelcomer``, ``!!embed <text>``, ``!!role @user <rolename>``, ``!!setnick @user <New nickname>``, ``!!serverinfo``, ``!!userinfo @user``, ``!!lock #channel or !!lock``, ``!!unlock #channel or !!unlock``, ``!!membercount``, ``!!say <text>``, ``!!kick @user``, ``!!mute @user <time in minutes>``, ``!!unmute @user``, ',inline = False)
      ...
      if reaction.emoji == '👥':
        ...
        embed.add_field(name = 'SEE',value ='``!!help``, ``!!google <anything>``, ``!!youtube <anything>``, ``!!botinvite``, ``!!ping``, ``!!serverinvite``, ``!!avatar or !!avatar @user``, ``!!meme``, ',inline = False)
        react_message = await client.send_message(user,embed=embed)
        await asyncio.sleep(30)
        await client.delete_message(react_message)
